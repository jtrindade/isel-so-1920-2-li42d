...
int main(int argc, char * argv[]) {
	...
	for (;;) {
		...
		int conn_fd;
		int nready = epoll_wait(events_fd, events, nclients, INFTIM);
		...
		for (int i = 0; i < nready; ++i) {
			...
			if (pev->data.fd == 0 && pev->events & EPOLLIN) {
				...
				conn_fd = accept(main_fd, (struct sockaddr *)&cli_addr, &cli_addr_len);
				if (conn_fd < 0) {
				...
				if (nclients == MAX_CLIENTS) {
					puts("** maximum connections exceeded ** ");
					close(conn_fd);
				} else {
					ev.data.fd = conn_fd;
					ev.events = EPOLLIN;
					if (epoll_ctl(events_fd, EPOLL_CTL_ADD, conn_fd, &ev) == -1) {
						error("ERROR on epoll_ctl for new connection");
					}
				}
			} else {
				...
				if (pev->events & (EPOLLIN | EPOLLERR)) {
					conn_fd = pev->data.fd;
					...
					if (len == 0 || (strncmp(buf, "stop", 4) == 0 && buf[4] < ' ')) {
						printf("** client %d closing **\n", conn_fd);
						
						ev.data.fd = conn_fd;
						ev.events = EPOLLIN;
						if (epoll_ctl(events_fd, EPOLL_CTL_DEL, conn_fd, &ev) == -1) {
							error("ERROR on epoll_ctl for closing connection");
						}
